    ...
    public class MapManager : MonoBehaviour
    {
        ...
        public int totalMapCount = 5;
        public static List<int> usedMapTypes = new List<int>();
        ...
        void InitializeMapSettings()
        {
            ...
            {
                usedMapTypes.Clear();
                for (int i = 0; i < totalMapCount; i++) availableTypes.Add(i);
            }

            int randomIndex = Random.Range(0, availableTypes.Count);
            savedMapType = availableTypes[randomIndex];
            usedMapTypes.Add(savedMapType);
        ...
        void SetupMapData()
        {
            ...
            switch (currentMapType)
            {
                ...
                case 3: GenerateTypeDMap(); break;
                case 4: GenerateTypeEMap(); break;
        ...
        void GenerateTypeAMap()
        {
            ...
            MapNode boss = CreateNode("Boss", 0, 4.5f, RoomType.Boss);         // 맨 위 긴 상자
        ...
        void GenerateTypeBMap()
        {
            ...
            MapNode boss = CreateNode("Boss", 0f, 4.5f, RoomType.Boss);
            MapNode shop = CreateNode("Shop", 0f, 2f, RoomType.Shop); // 상단 중앙 상점

            // 중앙 거대 방
            MapNode huge = CreateNode("Huge", 0f, -1f, RoomType.Huge);

            // 중간층 전투 방 (좌, 우)
            MapNode fightML = CreateNode("FML", -4f, -1f, RoomType.Fight);
            MapNode fightMR = CreateNode("FMR", 4f, -1f, RoomType.Fight);

            // 상층 전투 방 (좌, 우)
            MapNode fightTL = CreateNode("FTL", -3.5f, 2f, RoomType.Fight);
            MapNode fightTR = CreateNode("FTR", 3.5f, 2f, RoomType.Fight);

            // 하단 방사형 배치 (상호작용 2개 + 스폰)
            MapNode interactL = CreateNode("InteractL", -5f, -4f, RoomType.Interaction);
            MapNode interactR = CreateNode("InteractR", 5f, -4f, RoomType.Interaction);
        ...
        void GenerateTypeCMap()
        {
            ...
            // 상단 상점
            MapNode shop = CreateNode("Shop", 0f, 2.6f, RoomType.Shop);

            // (중앙 상단) 전투방 2개
            MapNode fightTL = CreateNode("FTL", -3f, 0.9f, RoomType.Fight);
            MapNode fightTR = CreateNode("FTR", 3f, 0.9f, RoomType.Fight);

            // 중앙 거대 방 
            MapNode huge = CreateNode("Huge", 0f, -0.6f, RoomType.Huge);

            // 좌우 상호작용 방 
            MapNode interactL = CreateNode("InteractL", -6f, -0.6f, RoomType.Interaction);
            MapNode interactR = CreateNode("InteractR", 6f, -0.6f, RoomType.Interaction);

            // 하단 전투방 2개
            MapNode fightBL = CreateNode("FBL", -3f, -2.3f, RoomType.Fight);
            MapNode fightBR = CreateNode("FBR", 3f, -2.3f, RoomType.Fight);
            ...
            // 상단 전투방들 - 중앙 거대 방
            Connect(fightTL, huge);
            Connect(fightTR, huge);
    ...
    void GenerateTypeDMap()
    {
        // 1. 노드 생성 (이미지 D 구도 기준 좌표 설정)
        MapNode boss = CreateNode("Boss", 0f, 5f, RoomType.Boss);
        MapNode shop = CreateNode("Shop", 0f, 2.5f, RoomType.Shop);
        ...
        // --- 왼쪽 라인 ---
        MapNode interactL = CreateNode("InteractL", -5f, 4f, RoomType.Interaction);
        MapNode fightTL = CreateNode("FTL", -5f, 2f, RoomType.Fight);
        MapNode fightML = CreateNode("FML", -5f, -1f, RoomType.Fight);
        MapNode fightBL = CreateNode("FBL", -5f, -4f, RoomType.Fight);

        // --- 오른쪽 라인 ---
        MapNode interactR = CreateNode("InteractR", 5f, 4f, RoomType.Interaction);
        MapNode fightTR = CreateNode("FTR", 5f, 2f, RoomType.Fight);
        MapNode fightMR = CreateNode("FMR", 5f, -1f, RoomType.Fight);
        MapNode fightBR = CreateNode("FBR", 5f, -4f, RoomType.Fight);
        ...
        // 왼쪽 세로 라인 연결
        Connect(interactL, fightTL);
        Connect(fightTL, fightML);
        Connect(fightML, fightBL);

        // 오른쪽 세로 라인 연결
        Connect(interactR, fightTR);
        Connect(fightTR, fightMR);
        Connect(fightMR, fightBR);

        // 가로 및 중앙 연결
        Connect(huge, fightML);
        Connect(huge, fightMR);
        Connect(spawn, fightBL);
        Connect(spawn, fightBR);
    }
    void GenerateTypeEMap()
    {
        // 1. 노드 생성 (보스는 중앙 상단, 경로는 지그재그)
        MapNode boss = CreateNode("Boss", 0f, 5f, RoomType.Boss);

        // 상단 라인 (상점은 보스 바로 아래, 옆에 전투방 하나)
        MapNode shop = CreateNode("Shop", 0f, 3f, RoomType.Shop);
        MapNode fightTop = CreateNode("FTop", 0f, 1f, RoomType.Fight);

        // 오른쪽 꺾이는 지점
        MapNode interactTop = CreateNode("InteractTop", 4f, 3f, RoomType.Interaction);

        // 중간 라인 (오른쪽에서 왼쪽으로 진행)
        MapNode fightMid = CreateNode("FMid", -4f, -1.5f, RoomType.Fight);
        MapNode interactMid = CreateNode("InteractMid", 4f, -1.5f, RoomType.Interaction);
        MapNode fightMidLeft = CreateNode("FMidL", 0f, -1.5f, RoomType.Fight);

        // 하단 라인 (왼쪽 아래로 꺾임)
        MapNode spawn = CreateNode("Spawn", -4f, -4f, RoomType.Spawn);

        // 2. 연결 설정 (ㄹ자 형태)
        Connect(boss, shop);
        Connect(shop, fightTop);
        Connect(shop, interactTop);

        Connect(fightMidLeft,interactMid);

        Connect(fightMid, fightMidLeft);
        Connect(fightTop, fightMidLeft);

        Connect(fightMid, spawn);
    }
